...
if exist ".git" (
    ...
    timeout /t 5 git config --local credential.helper store >nul 2>&1
    echo [DEBUG] Setting up GitHub URL rewriting...
    timeout /t 5 git config --local url."https://github.com/".insteadOf "git@github.com:" >nul 2>&1
    timeout /t 5 git config --local url."https://".insteadOf "git://" >nul 2>&1
    echo [DEBUG] Setting up GitHub credential path...
    timeout /t 5 git config --local credential.https://github.com.useHttpPath true >nul 2>&1
